/**
 * SpearFishing.js — ECJ Game Library
 * Mecânica de pesca primitiva: lança, mão, rede etc.
 * A narrativa é definida pelo jogo — a lib gerencia apenas a lógica.
 *
 ...
 *   "surfacing"  → { creature }           alvo apareceu na água
 ...
 *   const game = SpearFishing.create({ pool });
 *
 *   game.on("surfacing", ({ creature }) => { a11y.vibrate("bite"); speak("Alvo na água!"); });
...
export const SpearFishing = {
  create({
    ...
    if (!pool) throw new Error("[SpearFishing] pool é obrigatório.");
