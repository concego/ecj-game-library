/**
 * InteractionSequence.js — ECJ Game Library
 * Reconhece sequências de gestos e dispara eventos quando completadas ou falhas.
 * A narrativa é definida pelo jogo — a lib gerencia apenas o reconhecimento.
 *
 * REGRA DE OURO: sem áudio, sem visual. Emite eventos; o jogo decide o que fazer.
 *
 * Conceitos:
 *   - Step:     um gesto esperado na sequência { gesture, timeoutMs? }
 *   - Gesture:  string definida pelo jogo (ex: "shake", "tilt_left", "tilt_right")
 *   - Sequence: array de Steps que define um padrão completo
 *
 * Fluxo:
 *   IDLE → input(gesture) → avança step → ... → "completed" | "failed" | "timeout"
 *
 * Eventos emitidos (use .on(event, cb)):
 *   "started"    → { sequence }                  primeiro gesto correto recebido
 *   "progress"   → { step, total, gesture }       gesto correto, avança
 *   "completed"  → { sequence, elapsed }          sequência completa
 *   "failed"     → { expected, received, step }   gesto errado
 *   "timeout"    → { step, total }                tempo esgotado no step atual
 *   "reset"      → {}                             sequência reiniciada manualmente
 *
 * Uso básico:
 *
 *   const seq = InteractionSequence.create({
 *     sequence: [
 *       { gesture: "shake"      },
 *       { gesture: "tilt_left",  timeoutMs: 2000 },
 *       { gesture: "tilt_right", timeoutMs: 2000 },
 *     ],
 *     globalTimeoutMs: 5000,   // tempo total da sequência (opcional)
 *     strict: true,            // gestos errados falham (padrão: true)
 *   });
 *
 *   seq.on("completed", ({ elapsed }) => { // abrir fechadura, lançar magia... });
 *   seq.on("failed",    ({ expected, received }) => { // feedback negativo });
 *   seq.on("timeout",   ({ step }) => { // tempo esgotado });
 *
 *   // Integração com SensorKit:
 *   SensorKit.on("shake",      () => seq.input("shake"));
 *   SensorKit.on("tiltLeft",   () => seq.input("tilt_left"));
 *   SensorKit.on("tiltRight",  () => seq.input("tilt_right"));
 *   SensorKit.on("tiltForward",() => seq.input("tilt_forward"));
 */

const InteractionSequence = (() => {

  /**
   * Cria uma instância de reconhecimento de sequência.
   *
   * @param {Object}   opts
   * @param {Array}    opts.sequence         - Steps: [{ gesture, timeoutMs? }, ...]
   * @param {number}   [opts.globalTimeoutMs]- Timeout total da sequência (ms)
   * @param {boolean}  [opts.strict=true]    - Se true, gesto errado → "failed"
   *                                           Se false, gesto errado é ignorado
   * @param {boolean}  [opts.loop=false]     - Se true, reinicia após completar
   */
  function create(opts = {}) {
    const sequence        = (opts.sequence || []).map((s, i) => ({ ...s, index: i }));
    const globalTimeoutMs = opts.globalTimeoutMs || null;
    const strict          = opts.strict !== false;
    const loop            = opts.loop   || false;

    if (!sequence.length) throw new Error("InteractionSequence: sequence não pode ser vazia.");

    // ── Estado interno ────────────────────────────────────────────────────────
    let _listeners       = {};
    let _currentStep     = 0;
    let _state           = "idle";       // idle | running | completed | failed
    let _stepTimer       = null;
    let _globalTimer     = null;
    let _startedAt       = null;

    // ── EventEmitter ─────────────────────────────────────────────────────────
    function _emit(event, data) {
      (_listeners[event] || []).forEach(cb => cb(data));
    }

    function on(event, cb) {
      if (!_listeners[event]) _listeners[event] = [];
      _listeners[event].push(cb);
      return () => off(event, cb);
    }

    function off(event, cb) {
      if (!_listeners[event]) return;
      _listeners[event] = _listeners[event].filter(fn => fn !== cb);
    }

    // ── Timers ────────────────────────────────────────────────────────────────
    function _clearTimers() {
      if (_stepTimer)   { clearTimeout(_stepTimer);   _stepTimer   = null; }
      if (_globalTimer) { clearTimeout(_globalTimer); _globalTimer = null; }
    }

    function _startStepTimer(step) {
      if (!step.timeoutMs) return;
      _stepTimer = setTimeout(() => {
        _clearTimers();
        _state = "idle";
        _currentStep = 0;
        _emit("timeout", { step: step.index, total: sequence.length });
      }, step.timeoutMs);
    }

    function _startGlobalTimer() {
      if (!globalTimeoutMs) return;
      _globalTimer = setTimeout(() => {
        _clearTimers();
        _state = "idle";
        _currentStep = 0;
        _emit("timeout", { step: _currentStep, total: sequence.length });
      }, globalTimeoutMs);
    }

    // ── Core ──────────────────────────────────────────────────────────────────

    /**
     * Envia um gesto para o reconhecedor.
     * @param {string} gesture - nome do gesto (ex: "shake", "tilt_left")
     */
    function input(gesture) {
      if (_state === "completed" || _state === "failed") return;

      const expected = sequence[_currentStep];

      // Gesto correto
      if (gesture === expected.gesture) {

        // Primeiro step — inicia sequência
        if (_currentStep === 0) {
          _state     = "running";
          _startedAt = Date.now();
          _startGlobalTimer();
          _emit("started", { sequence });
        }

        _clearTimers();  // cancela timer do step anterior

        _emit("progress", {
          step:    _currentStep,
          total:   sequence.length,
          gesture,
        });

        _currentStep++;

        // Sequência completa
        if (_currentStep >= sequence.length) {
          _clearTimers();
          const elapsed = Date.now() - _startedAt;
          _state = loop ? "idle" : "completed";
          if (loop) _currentStep = 0;
          _emit("completed", { sequence, elapsed });
          return;
        }

        // Inicia timer do próximo step
        _startStepTimer(sequence[_currentStep]);
        return;
      }

      // Gesto errado
      if (strict && _state === "running") {
        _clearTimers();
        _emit("failed", {
          expected: expected.gesture,
          received: gesture,
          step:     _currentStep,
        });
        _state       = "idle";
        _currentStep = 0;
        return;
      }

      // Modo não-strict: ignora gestos errados enquanto não iniciou
      // mas falha se já estava no meio da sequência
      if (!strict && _state === "running") {
        _clearTimers();
        _emit("failed", {
          expected: expected.gesture,
          received: gesture,
          step:     _currentStep,
        });
        _state       = "idle";
        _currentStep = 0;
      }
      // Se idle e não-strict, ignora o gesto errado silenciosamente
    }

    /**
     * Reinicia a sequência manualmente.
     */
    function reset() {
      _clearTimers();
      _currentStep = 0;
      _state       = "idle";
      _startedAt   = null;
      _emit("reset", {});
    }

    /** Estado atual: idle | running | completed | failed */
    function state() { return _state; }

    /** Step atual (0-based) */
    function currentStep() { return _currentStep; }

    /** Quantidade de steps */
    function totalSteps() { return sequence.length; }

    /** Progresso 0..1 */
    function progress() {
      return sequence.length ? _currentStep / sequence.length : 0;
    }

    const api = {
      on, off,
      input, reset,
      state, currentStep, totalSteps, progress,
    };
    return api;
  }

  // ── Factory helpers ────────────────────────────────────────────────────────

  /**
   * Atalho para sequências simples sem timeout por step.
   * @param {string[]} gestures  - ex: ["shake", "tilt_left", "tilt_right"]
   * @param {Object}   [opts]    - globalTimeoutMs, strict, loop
   */
  function fromGestures(gestures, opts = {}) {
    return create({ ...opts, sequence: gestures.map(g => ({ gesture: g })) });
  }

  return { create, fromGestures };
})();
