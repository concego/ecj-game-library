 ...
 * Opportunity window mechanic: primitive fishing, spear hunting, ambush, etc.
 * The narrative is defined by the game — the lib manages only timing and reaction logic.
 *
 * GOLDEN RULE: no audio, no visuals. Emits events; the game decides what to do.
 *
 * Flow:
 *   IDLE → SURFACING → HIT | MISS
 *   Attack with no target present → SPOOKED (penalty)
 *
 * Target profile parameters (via CreatureProfile):
 *   surfaceWindowMs  → time the target is visible (ms)
 *   cooldownMs       → interval between normal appearances (ms)
 *   spookCooldownMs  → cooldown after spooking (ms) — should be > cooldownMs
 *   weight           → appearance probability weight
 *
 * Emitted events (use .on(event, cb)):
 *   "surfacing"  → { creature }           target appeared in the field
 *   "submerged"  → { creature, reason }   target disappeared ("timeout" | "spooked")
 *   "hit"        → { creature }           player hit the target
 *   "miss"       → { creature }           target fled before the attack
 *   "spooked"    → {}                     attack with no target — penalty
 *   "cooldown"   → { ms }                 cooldown started, reports duration
 ...
 *   game.on("spooked",   ()            => { speak("You spooked the targets!"); });
 *
...
export const TimedStrike = {
  create({
    ...
    if (!pool) throw new Error("[TimedStrike] pool is required.");
    ...
    function _nextRound() {
      ...
      // Target disappears after surfaceWindowMs with no attack
    ...
    function attack() {
      ...
      // If _state === "surfacing" but _current was already cleared (race), ignore silently
    }
