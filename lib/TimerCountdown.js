...
export const TimerCountdown = {
  ...
  create({
     ...
     * @param {number} secs seconds to add
     */
    function addTime(secs) {
      // Extend total so pct() stays ≤ 1.0
      _total     += secs;
      _remaining  = Math.min(_remaining + secs, _total);
      // If it was urgent but bonus pushed above threshold, allow re-firing
      if (_remaining > urgencyThreshold) _urgentFired = false;
      _emit("added", { added: secs, remaining: _remaining, total: _total });
    }
