 ...
 * Abstracts DeviceOrientation (tilt) and DeviceMotion (shake)
 * Compatible with browser (GitHub Pages) and Capacitor (APK)
 *
 * Axes (portrait):
 *   beta  → forward/back  (-180..180)
...
export const SensorKit = (() => {
  ...
  const CONFIG = {
    tiltForwardThreshold:  -18,   // beta negative → tilted forward
    tiltBackThreshold:      18,   // beta positive → tilted back
    tiltLeftThreshold:     -18,   // gamma negative → left
    tiltRightThreshold:     18,   // gamma positive → right
  ...
  function enableDesktopFallback() {
    ...
    console.info("[SensorKit] Desktop: ↑↓←→ = tilt | Space = shake");
  }
